...
public class EventService
{
    ...
    // Checks if a column exists in a table by trying multiple case variations (handles Access case-insensitivity)
    private bool ColumnExists(OleDbConnection conn, string tableName, string columnName)
    {
        try
        {
            // Try different case variations: original, lowercase, uppercase, and PascalCase
    ...
    // Loads all events from database, applies user filtering based on role, merges shared calendar events,
    // fixes Hebrew encoding, ensures all required columns exist, and sorts by date/time before returning
    public DataTable GetAllEvents(int? userId = null)
    {
        ...
        try
        {
            ...
            {
                // Open database connection and log success or failure
                try
                {
                    con.Open();
                    try
                ...
                {
                    // Log connection errors and rethrow to caller
                    try
                    {
                        LoggingService.Log("EventService",
                ...
                // Verify CalendarEvents table exists before querying
                ...
                // Count total events for logging purposes
                try
                {
                ...
                {
                    // Log counting errors but continue execution
                    try
                    {
                        LoggingService.Log("EventService",
                ...
                // Query all events from CalendarEvents table
                string sql = @"
SELECT *
FROM CalendarEvents
ORDER BY Id";
                ...
                // Determine correct column name for username (handles case variations)
                ...
                // Load all users into dictionary for quick lookup by ID
                ...
                if (usersTableExists)
                {
                    try
                    {
                        ...
                        // Build dictionary mapping user ID to username with Hebrew encoding fix
                    ...
                    {
                        // Log user loading errors but continue without user names
                        try
                        {
                            LoggingService.Log("EventService",
                ...
                // Fix Hebrew encoding for all text columns in events
                ...
                // Populate UserName column from dictionary and set default category if missing
                foreach (DataRow row in dt.Rows)
                {
                    // Look up username by user ID from dictionary
                    ...
                    // Set default category "אחר" if category is missing or empty
                    if (row.Table.Columns.Contains("Category"))
                    {
                ...
                // Check if shared calendar tables exist before attempting to merge shared events
...
ORDER BY Id";
                    ...
                    // Normalize column names in shared events table (handle case variations and alternative names)
                    ...
                    // Map Date column to EventDate if EventDate doesn't exist
                    if (!hasEventDate)
                    {
                    ...
                    // Map Time column to EventTime if EventTime doesn't exist
                    if (!hasEventTime)
                    {
                    ...
                    // Map lowercase category to Category if Category doesn't exist
                    if (!hasCategory)
                    {
                    ...
                    // Map CreatedBy to UserId if UserId doesn't exist
                    if (!hasUserId)
                    {
                    ...
                    // Add UserName column to shared events table
                    if (!sharedDt.Columns.Contains("UserName"))
                        sharedDt.Columns.Add("UserName", typeof(string));

                    // Process shared events: fix encoding, populate usernames, set default categories
                    foreach (DataRow row in sharedDt.Rows)
                    {
                        // Fix Hebrew encoding for text fields
                        if (row.Table.Columns.Contains("Title"))
                            row["Title"] = Connect.FixEncoding(Convert.ToString(row["Title"]));
                        if (row.Table.Columns.Contains("EventTime"))
                            row["EventTime"] = Connect.FixEncoding(Convert.ToString(row["EventTime"]));
                        ...
                        // Extract user ID from UserId or CreatedBy column
                        int uid = -1;
                        if (row.Table.Columns.Contains("UserId") && row["UserId"] != DBNull.Value && row["UserId"] != null)
                        {
                            try
                        ...
                        // Look up username from dictionary
                    ...
                    // Merge shared events into main events table, handling missing columns with default values
                    foreach (DataRow sharedRow in sharedDt.Rows)
                    {
                        ...
                        foreach (DataColumn col in dt.Columns)
                        {
                            ...
                            {
                                // Copy value if exists, otherwise use type-appropriate default
                            ...
                            {
                                // Column doesn't exist in shared table, use default value
                                if (col.DataType == typeof(int))
                                    newRow[col.ColumnName] = 0;
                                else if (col.DataType == typeof(DateTime))
                                    newRow[col.ColumnName] = DateTime.MinValue;
                ...
                // Sort events by date and time (newest first) if date/time columns exist
                if (dt.Columns.Contains("EventDate") && dt.Columns.Contains("EventTime"))
                {
                    ...
                    // Copy sorted rows to new table with default values for missing data
                ...
                // Alternative sorting using Date/Time columns if EventDate/EventTime don't exist
        ...
        {
            // Log all errors with full details but don't throw (return empty table instead)
            try
            {
                LoggingService.Log("EventService",
    ...
    // Ensures all required columns exist in the DataTable, adding them with appropriate types and default values if missing
    private void EnsureRequiredColumnsInTable(DataTable dt)
    {
        ...
        foreach (string colName in requiredColumns)
        {
            if (!dt.Columns.Contains(colName))
            {
                ...
                // Fill existing rows with appropriate default values
                foreach (DataRow row in dt.Rows)
                {
                    if (colName == "Id" || colName == "UserId")
                        row[colName] = 0;
